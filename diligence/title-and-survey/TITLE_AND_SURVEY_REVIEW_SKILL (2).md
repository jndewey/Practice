...
description: "Comprehensive title insurance and survey review skill for commercial real estate loans from lender's perspective. Use when reviewing title commitments, annotating commitments for title agents, reviewing ALTA/NSPS surveys, analyzing Schedule B exceptions, requesting endorsements, or preparing title objection letters or memos. Includes mandatory character-by-character legal description comparison (delivered as separate landscape document), Florida-specific endorsement availability, annotation formatting, structured analysis framework, and special situation guidance."
---
...
This skill governs three distinct output types:
...
3. **Legal Description Comparison Document** — mandatory landscape-format Word document containing the character-by-character comparison of legal descriptions across all available sources (title policy, standalone legal, survey). Always produced when both a title commitment/pro forma and survey are available. See Section B.5.B.

Use the appropriate output based on the instruction. If both a commitment/pro forma and survey are provided, the Legal Description Comparison Document is ALWAYS produced alongside the Title and Survey Review Memorandum. If both annotation and memo are requested, produce all three.
...
A. LEGAL DESCRIPTION COMPARISON (MANDATORY — SEE B.5.B)
-------------------------------------------------------
**This section ALWAYS includes the full character-by-character
comparison produced per Section B.5.B below. It is delivered
as a separate landscape-format document alongside the main memo.**

COMPARISON DOCUMENT INCLUDES:
- Source identification (all documents compared)
- Image resolution / OCR limitations noted
- Section 1: Preamble / Parcel Identification comparison table
- Section 2: Point of Commencement comparison table
- Sections 3–N: Each metes-and-bounds call as a separate
  comparison table (bearing, course, distance, to-point)
- Discrepancy Summary with classification
  (Substantive / Formatting / Verify)
- Mathematical Closure Check
- Recommendations and corrective actions

SUMMARY TABLE (always included in main memo as well):

| Element | Schedule A / Policy | Survey | Match? |
|---------|-----------|--------|--------|
| Township | | | |
| Range | | | |
| Section | | | |
...
| Each M&B Call | | | |
| Total Acreage | | | |
| Overall Status | | | MATCH / DISCREPANCIES FOUND |
...
[Reference Legal Description Comparison Document for any
legal description discrepancies identified]
...
#### B. Legal Description Comparison (CRITICAL — MANDATORY DELIVERABLE)

**This comparison is a mandatory deliverable for every title and survey review. It MUST be produced as a separate landscape-format Word document accompanying the main review memorandum whenever both a title commitment/pro forma and a survey are available for review.**

##### B.5.B.1 Overview and Purpose

The legal description comparison serves as the definitive record that lender's counsel has verified the legal description is consistent across all loan documents, the title policy, and the survey. Discrepancies in legal descriptions — even minor ones — can create title insurance gaps, cloud the lender's security interest, or cause recording rejections.

##### B.5.B.2 Source Documents

Compare the legal description across ALL available sources. Common sources include:
- Title commitment or pro forma loan policy (Schedule A / Exhibit A)
- Standalone replacement legal description (if provided separately)
- ALTA/NSPS survey face (Legal Description section)
- Mortgage / deed of trust (Exhibit A)
- Loan agreement (property description)

If a standalone replacement legal has been provided to replace the commitment/pro forma legal, note this and use the standalone as the primary comparison document. Always note the relationship between documents (e.g., "The standalone legal will replace the pro forma Exhibit A").

##### B.5.B.3 Extraction Methodology

**Text-based documents (commitment, pro forma, standalone legal):**
- Extract the legal description verbatim, preserving all punctuation, spacing, degree symbols, and special characters.
- If the document is image-based (scanned PDF), use OCR and note any characters that are ambiguous.
- Preserve the document's exact formatting of bearing notations (e.g., S88°47'06"W vs. S 88°47'06" W).

**Survey:**
- The legal description on the survey face is typically in very small print. If working from a scanned image, note the resolution limitation.
- Request a high-resolution scan if the survey image is insufficient for character-level verification.
- Also cross-reference bearings and distances on the survey face against the plotted boundary lines and dimension callouts on the survey drawing itself.

##### B.5.B.4 Comparison Structure — ALWAYS USE THIS FORMAT

The comparison document MUST be organized as follows:

**Document Header:**
- Title: "LEGAL DESCRIPTION: CHARACTER-BY-CHARACTER COMPARISON"
- Header: "PRIVILEGED AND CONFIDENTIAL | ATTORNEY WORK PRODUCT"
- Source documents identified with full names and descriptions
- Image resolution limitations noted (if applicable)
- Date of comparison

**Section 1: Preamble / Parcel Identification**

Compare in a table with columns: Element | Source 1 (Policy/Standalone) | Source 2 (Survey) | Status | Notes

Elements to compare:
- Parcel identifier (Parcel A, Parcel 1, etc.)
- Lot/Block reference
- Section, Township, Range
- Subdivision name (exact spelling)
- Plat Book and Page reference (including punctuation)
- County and State
- Introductory clause punctuation ("DESCRIBED AS FOLLOWS" — semicolon vs. colon vs. comma)

**Section 2: Point of Commencement (if applicable)**

If the legal description begins with a "COMMENCE AT" clause (as opposed to starting directly at the Point of Beginning), compare:
- Starting point description
- Initial bearing, course description, and distance to the Point of Beginning
- Point of Beginning identification language

**Sections 3 through N: Each Metes-and-Bounds Call**

Create a SEPARATE comparison table for EACH call in the legal description. For each call, compare:
- **Bearing**: Character-by-character, including degree symbol (°), minute mark ('), second mark ("), direction letters (N/S/E/W), and any spaces between components
- **Course description**: "ALONG THE [description]" language
- **Distance**: Exact footage to hundredths
- **To-point description**: The point or line reached by the call
- **Continuation language**: "THENCE CONTINUE" vs. "THENCE RUN" vs. other verb forms

Mark each element as:
- **MATCH** — Identical in substance and form
- **DISCREPANCY** — Difference identified (red highlight)
- **VERIFY** — Cannot be confirmed due to image quality or ambiguity

**Discrepancy Summary Section**

After all calls are compared, provide a summary that:
1. States the overall assessment (substantively identical / discrepancies found)
2. Lists each discrepancy with classification:
   - **Substantive Discrepancy**: Different bearing angle, different distance, different lot/block, missing call — MUST BE CORRECTED
   - **Formatting Discrepancy**: Spacing in bearings, punctuation differences, abbreviation variations — note but typically no action required
   - **Possible Typographical Error**: Misspelled words, missing spaces, transposed characters — VERIFY against original and correct if confirmed
3. For each discrepancy, state which document should be corrected and how

**Mathematical Closure Check**

Include a closure check:
- List all boundary calls from the Point of Beginning back to the Point of Beginning
- Verify north-south balance (sum of southerly distances vs. sum of northerly distances)
- Verify east-west balance (sum of westerly distances vs. sum of easterly distances)
- Note whether minor closure discrepancies are consistent with the non-rectangular shape described by differing bearing angles
- State whether a licensed surveyor has certified closure

**Recommendations**

Numbered list of specific corrective actions required before closing, including:
- Typographical corrections needed (specify document and exact correction)
- Formatting harmonization (if desired, noting it is not required)
- High-resolution scan requests
- Confirmation that the correct legal description is used in the final policy

##### B.5.B.5 Output Format Requirements

- **Format**: Word document (.docx), landscape orientation
- **Font**: Comparison tables use monospaced font (Courier New, size 8-9pt) for the legal description text to facilitate character-by-character visual comparison
- **Table styling**: Header rows in dark blue with white text; MATCH rows in default styling; DISCREPANCY rows with light red background; VERIFY rows with light yellow background
- **Filename**: `Legal_Description_Comparison_[Project_Name].docx`
- **Delivery**: Always delivered alongside the main Title and Survey Review Memorandum; never omitted

##### B.5.B.6 Common Discrepancy Patterns

The following patterns appear frequently and should be checked in every comparison:

| Pattern | Example | Classification | Action |
|---------|---------|---------------|--------|
| Bearing spacing | S88°47'06"W vs. S 88°47'06" W | Formatting | Note; no correction required |
| Plat reference commas | "Plat Book 5 at Page 10" vs. "Plat Book 5, at Page 10," | Formatting | Note; no correction required |
| Intro punctuation | "FOLLOWS;" vs. "FOLLOWS:" | Formatting | Note; no correction required |
| Missing word space | "TOTHE" vs. "TO THE" | Typographical | Verify and correct |
| Misspelled words | "BEGJNNING" vs. "BEGINNING" | Typographical | Verify and correct |
| Compressed words | "MIAMILAND" vs. "MIAMI LAND" | Typographical | Verify and correct |
| NE1/4 vs. NE 1/4 | Spacing around fraction | Formatting | Note; no correction required |
| Block I vs. Block 1 | Roman numeral vs. Arabic | Substantive | Verify against plat |
| Distance difference | 445.19 vs. 445.91 | Substantive | MUST CORRECT — verify against survey |
| Bearing difference | S88°47'06"W vs. S88°47'16"W | Substantive | MUST CORRECT — verify against survey |
| Missing call | Legal has 6 calls; survey has 5 | Substantive | MUST CORRECT — legal may not close |
| Different lot reference | Lot 3 vs. Lot 4 | Substantive | MUST CORRECT — wrong property |

##### B.5.B.7 When Survey Is Not Yet Available

If only the title commitment/pro forma is available (no survey yet):
- Extract and transcribe the legal description verbatim in the main memo
- Note that the character-by-character comparison is PENDING receipt of the survey
- Add to Comments to Title Agent: "Legal description comparison pending receipt of certified ALTA/NSPS survey"
- Flag as an open item in the Title Objection Summary

When the survey is subsequently received, produce the full comparison document as a supplemental deliverable.
...
- [ ] Substantive legal description discrepancy between survey and title policy (different bearings, distances, lot references, or missing calls — see Legal Description Comparison Document)
...
- [ ] High-resolution PDF/scan of survey (minimum 300 DPI) for legal description character-by-character comparison
...
|---------|------|---------|
| 3.1 | 2/19/2026 | Added mandatory Legal Description Character-by-Character Comparison deliverable (Section B.5.B); expanded from brief checklist to comprehensive methodology with structured output format, comparison tables, discrepancy classification system, mathematical closure check, and output format requirements; updated Part IV template in B.1 to reference new comparison document; comparison is now a separate landscape-format Word document delivered alongside every title and survey review memo |
